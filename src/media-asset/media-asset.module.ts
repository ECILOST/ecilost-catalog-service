...
import { AzureBlobMediaStorage } from './storage/azure-blob-media-storage.js';
...
@Module({
  ...
  providers: [
    MediaAssetService,
    // El caso de uso depende de los dos puertos, no de Prisma ni del SDK de Azure. Cambiar de
...

    {
      ...
      //
      // La pareja se arma aqui dentro y el adaptador de Azure no se registra como proveedor
      ...
      useFactory: (config: CatalogConfig) =>
        new CachingMediaStorage(new AzureBlobMediaStorage(config)),
